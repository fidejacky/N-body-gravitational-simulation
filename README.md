...
nvcc -O2 -std=c++17 -arch=native src\nbody_naive.cu -o results\nbody_naive.exe
```
...
- `-arch=native` compiles the kernel for whatever GPU is physically in the machine running the build. Swap it for an explicit target like `-arch=sm_75` to pin a specific compute capability instead.
...
**Stage 1 (CUDA scaffold)** does not have the naive kernel implemented yet, so it just confirms the CUDA build pipeline works end to end:
...
```text
Stage 1 CUDA scaffold ready; the naive kernel implementation will be added next.
```
